...
popu_ag(:, 2) = sum(popu_ag(:, 2:3), 2); %5-17
...
now_date = datetime(2022,12,4);%datetime(now,'ConvertFrom','datenum')-1; %datetime(2021, 11, 21);
...
un_array = popu*0 + [100 125 150 175 200];
un_list = [1 2 3 4 5];
% halpha_list = 0.9:0.02:0.98;
shifts = [0]; %[0 3*7 -10*7 -5*7]; %Based on Ausie data & a zero value
past_seasons = -1:1:3; %0 is for 2021-22. and 1 to 5 is for 2019 to 2015 seasons (We can choose less)
...
%% vaccine effect adjustment
% vacc_A_B_C_D starts on Aug 8, 2022. Need to pad to start on zero date
vax_reaction_time = 14;
pad_days = days(datetime(2022, 8, 8) - zero_date) + vax_reaction_time;
vacc_ag = padarray(vacc_A_B_C_D, [0 pad_days 0], 0,"pre");
...
scen_ids = {'A'; 'B'; 'C'; 'D'};
dat_type = {'hosp';'death'};
scen_names = {'highVE_optImm'; 'highVE_pesImm'; 'lowVE_optImm'; 'lowVE_pesImm'};
...
for ss = 1:length(scen_ids)
    for dt = 1:length(dat_type)
        ...
        for cid = 1:length(popu)
            ...
            bad_sims = max_peaks > 40*prev_peaks | max_peaks < prev_peaks/400;
...
%% Save data
save ../../../../data_dumps/flu_round_3.mat
