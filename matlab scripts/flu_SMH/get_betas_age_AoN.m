function [beta_all_cell, ci, fittedC, Mdl, all_M] = get_betas_age_AoN(all_del_data, immune_infec, rel_lineage_rates, P_ag, death_data_ag, alpha_l, k_l, jp_l, window_size, ret_conf, compute_region, lags, M_0, vac_ag, un_ag, popu_ag)

% all_del_data: new cases in region 1st dim, caused by variant in 2nd dim at time 3rd dim to
% age group in 4th dim

% immun_infec: same diemsnions as all_del_data. Pass zeros to ignore
% different protection levels

% vac_ag is 3 dimensional: num of regions x num of days x num of age groups

% rel_lineage_rate: 1, P_ag = 0, lags = 0

% compute_region: all 1s nn x 1 matrix
...
for g = 1:ag
    ...
    for j=1:nn
        ...
        if compute_region(j) < 1 || death_data(j, end) < 1% Do not compute for region that are not specified
            continue;
        end
        ...
        for t = skip_days+jk+1:maxt-1
            ...
            M = (M_0(g)) + (death_data(j, t-1).*un_ag(g) + vac_ag(j, t-1, g))./popu_ag(j,g);
            X(t-jk-skip_days, :) = (1 - M).*[Ikt] ;
