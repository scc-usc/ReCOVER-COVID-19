...
now_date = datetime(2022, 7, 3); % Use data up to this date
% This round's data cutoff is on June 5, 2022
% T_corr = week of now_date - data cutoff date
T_corr = 4; % these many previous weeks will be appended to projections to keep the dates consistent
...
all_boosters_ts{4} = get_booster_preds(0*extra_dose_age_est(:, 1:second_start_day, :), all_boosters_ts{3}, pop_by_age, 0.9, 120, 1);
all_boosters_ts{5} = get_booster_preds(0*extra_dose_age_est, all_boosters_ts{4}, pop_by_age, 0.85/0.9, 120, 1);
all_boosters_ts{5}(:, :, 4) = all_boosters_ts{5}(:, :, 4).*15/(15+32); % Only 50+ are allowed third boosters
third_booster = all_boosters_ts{5};
...
ag_wan_param_list = repmat([30*4 30*5], [5 1]); % In days
...
all_boosters_ts{5} = third_booster;
...
scen_names = {'restBoo_noVar'; 'restBoo_Var'; 'broadBoo_noVar'; 'broadBoo_Var'};
...
%%
save ../../../../../code/data_dumps/round_14_large_update.mat
...
if write_data == 1
    ...
    full_T = [T; Tc; Tm; Tmc; us_T; us_Tc; us_Tm; us_Tmc; Tp1; Tp2];
