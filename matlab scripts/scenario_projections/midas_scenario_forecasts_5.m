...
temp = interp1([2, 9.5, 19.5, 29.5, 39.5, 49.5, 60, 69.5, 79.5, 85], cum_popu', target_age_medians);
popu_for_vacc = diff([zeros(1, 56); temp]);
[vacc_given_by_age_low] = get_vacc_admin(vacc_perc, 0.68, final_cov1, 12, popu_for_vacc, vacc_fd, 200); % For ~68%
[vacc_given_by_age_high] = get_vacc_admin(vacc_perc, 0.83, final_cov2, 12, popu_for_vacc, vacc_fd, 200); % For ~83%
%%
horizon = 200;
smooth_factor = 14;
...
for jj=1:length(sel_idx)
    ...
    temp = rat_data(cid, 1:thisday)*0;
    temp(tvals) = 1./(1+exp(-st_est));
    yy1 = smooth(temp, 14); yy2 = [yy1 yy1];
...
R0_var = [2.5 3]' * [1.3 1.5];
Target_Rt = ones(length(popu), 1) * R0_var(:)'; Rt_final_scens = [1 2 3 4];
