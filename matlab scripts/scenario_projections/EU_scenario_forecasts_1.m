...
early_boost = boost_prop60.*sum(get_booster_preds(extra_dose_age_est.*0, all_boosters_ts{3}, popu, 0.5, 180, 1), 3);
late_boost = boost_prop60.*sum(get_booster_preds([extra_dose_age_est.*0 zeros(ns, 90)], all_boosters_ts{3}, popu, 0.5, 180, 1), 3);
%%
all_boosters_ts{4} = early_boost;
all_boosters_ts{5} = 0*all_boosters_ts{4};

fifth_dose_BD = 0*early_boost; 
% countries (Greece and Netherlands) with ongoing 4th dose will have fifth dose in scenarios B and D
% identical to late campaign
idx = contains(countries, {'Greece', 'Netherlands'});
fifth_dose_BD(idx, :) = late_boost(idx, :);
...
temp(:, :, 1) = early_boost(:, 1:T+horizon, 1);
early_boost = temp;
...
temp = zeros(ns, T+horizon, ag);
temp(:, :, 1) = late_boost(:, 1:T+horizon, 1);
late_boost = temp;

temp = zeros(ns, T+horizon, ag);
temp(:, :, 1) = fifth_dose_BD(:, 1:T+horizon, 1);
fifth_dose_BD = temp;
...
cross_protect(omic_idx, :) = 0.5;
cross_protect(omic_idx, omic_idx) = 1;
cross_protect(wildcard_idx, omic_idx) = 0.7;
cross_protect(wildcard_idx, pre_omic_idx) = 0.7*0.5;
cross_protect(wildcard_idx, wildcard_idx) = 1;

rel_vacc_effi = ones(length(lineages), 1);
rel_vacc_effi(omic_idx) = 0.5;
rel_vacc_effi(wildcard_idx) = 0.5;
rel_booster_effi = rel_vacc_effi*0 + 1;
rel_booster_effi(omic_idx) = 0.8;
rel_booster_effi(wildcard_idx) = 0.8;
...
%% Scenario A. Slow waning, summer booster campaign
tic;
T = thisday;
disp('Executing Scenario A');

[X1, X2, X3, X4, X5, X6, X7] = ndgrid(un_list, var_prev_q, rlag_list, num_wan(1), mobility_scenarios(1), esc_param2_idxs, booster_imm_idxs);
scen_list = [X1(:), X2(:), X3(:), X4(:), X5(:), X6(:) X7(:)];

all_boosters_ts{4} = early_boost; all_boosters_ts{5} = 0*early_boost;
...
toc;
%% Scenario B. Slow waning, autumn booster campaign	
...
all_boosters_ts{4} = late_boost; all_boosters_ts{5} = fifth_dose_BD;
...
[X1, X2, X3, X4, X5, X6, X7] = ndgrid(un_list, var_prev_q, rlag_list, num_wan(2), mobility_scenarios(1), esc_param2_idxs, booster_imm_idxs);
scen_list = [X1(:), X2(:), X3(:), X4(:), X5(:), X6(:) X7(:)];

all_boosters_ts{4} = early_boost; all_boosters_ts{5} = 0*fifth_dose_BD;
...
%             [~, thisdata] = weighted_quantile(thisdata, 0.5, ww);
            ...
            if dt==1
                all_quant_diff(ss, dt, cid, :, :) = diffdata;
            else
                all_quant_diff(ss, dt, cid, :, :) = diffdata(3:5:end, :);
            end
        end
    end
end
...
%% Quant plots
%cid = popu>-1;
cid = 10; %cid = find(contains(countries, 'United Kingdom'));
...
%%
save ../../../../../code/data_dumps/EU_round_1_large2.mat
