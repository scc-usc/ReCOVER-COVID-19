function [all_betas, all_cont, fittedC] = multivar_beta_wan_custom(all_vars_data, F, popu, k_l, alpha_l, ...
    jp_l, un_fact, vac_ag, ag_case_frac, ag_popu_frac, waned_impact, ag_wan_lb, booster_age, booster_delay)
...
if nargin < 14  % booster mean delay from second dose
    booster_delay = 7*30;
end
...
waned_popu = zeros(length(popu), T, ag);
...
for j=1:length(popu)
    ...
    % Regain immunity if booster applied
    for idx = 1:ag
        xx = newM(idx, :)';
%        waned_popu(j, :, idx) = (squeeze(waned_impact(:, :, idx))*xx);
        for t=1:T
            if t > booster_delay
                xx(t-booster_delay) = xx(t-booster_delay) - new_boosters(j, t, idx)/popu(j);
                xx(t) = xx(t) + new_boosters(j, t, idx)/popu(j);
            end
