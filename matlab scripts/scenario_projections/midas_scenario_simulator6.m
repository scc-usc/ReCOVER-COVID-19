...
for simnum = 1:size(scen_list, 1)
    ...
    %%% Cases
    un = un_array(:, scen_list(simnum, 1));
    variant_fact = zeros(size(data_4_s)); variant_fact(:, end) = repmat(linspace(0, 0.05, 1), [56 1]);
    %variant_fact = movmean(variant_fact, 7, 2);
    ...
    beta_nvar = var_ind_beta_un(data_nvar_s(:, 1:(end-7*(ll))), 0, best_param_list_nv(:, 3)*0.1, best_param_list_nv(:, 1), un, popu, best_param_list_nv(:, 2), 0, compute_region, [], vacc_pre_immunity + un.*data_var_s(:, end));
    beta_var = cellfun(@(x)(trans_adv*x), beta_nvar, 'UniformOutput', false);
    ...
    dh_rate_change = get_rate_change_vac(dr_by_age, popu_for_vacc', 0.90, zeros(length(popu), horizon), vacc_given_by_age);
    h_rate_change = get_rate_change_vac(hr_by_age, popu_for_vacc', 0.90, zeros(length(popu), horizon), vacc_given_by_age);
    ...
    for jj = 1:length(death_rates_list)
        ...
        dh_rate_change1 = [dh_rate_change1].*this_rate_change(:, 1:(size(dh_rate_change1, 2)));
        dh_rate_change1 = dh_rate_change1./dh_rate_change1(:, 1);
