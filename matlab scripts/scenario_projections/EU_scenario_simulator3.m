...
parfor simnum = 1:size(scen_list, 1)
   ...
   esc_param = 0.5; % For omicron over delta
    ...
    cross_protect(wild_card_locs,wild_card_locs) = 1;

    for ww = 2:length(wild_card_locs)
        cross_protect(wild_card_locs(ww), wild_card_locs(wild_card_locs < wild_card_locs(ww))) ...
            = esc_param_w*cross_protect(wild_card_locs(ww-1), wild_card_locs(wild_card_locs < wild_card_locs(ww)));
       % cross_protect(wild_card_locs(ww),wild_card_locs(ww)) = 1;
    end
    
    

    rel_booster_effi = cell(length(all_boosters_ts      ), 1); fd_effi = 0.35;
    ...
    for bb=1:length(all_boosters_ts)
        ...
        for ww = 1:length(wild_card_locs)
            rel_booster_effi{bb}(wildcard_idx(ww)) = b_effi +(esc_param_w)*(1-b_effi);
        end
    end
%     if reform_b_effi > 0
%         rel_booster_effi{4}(:) = reform_b_effi; % reformulated booster is better!!
%     end
    
    rel_lineage_rates = ones(nl, 1);
    for ww = 1:length(wild_card_locs)
        rel_lineage_rates(wild_card_locs(ww)) = (severity)^ww;
    end
...
%     end
    ...
    for jj=1:ns
        ...
        if this_Rt(jj, foc_idx) > maxR % Enforce that the VoC (BA.2.75) does not have too high R advantage
            all_betas{foc_idx}{jj} = all_betas{foc_idx}{jj}* maxR/this_Rt(jj, foc_idx);
            this_Rt(jj, foc_idx) = maxR;
        end

        for ww = 1:length(wild_card_locs)
            all_betas{wild_card_locs(ww)}{jj} = all_betas{high_lin}{jj};
        end
    end
