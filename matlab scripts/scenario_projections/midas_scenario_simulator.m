...
for simnum = 1:size(scen_list, 1)
    ...
    vac_distributed_ts = (1./vacc_coverage)*vacc_future;
    zero_available = (latest_vac./(vacc_coverage+0.1) + cumsum(vac_distributed_ts, 2)) > popu;
    vac_distributed_ts(zero_available) = 0;
    
    vac_effi = equiv_vacc_effi*vacc_coverage;
    vac_available = [latest_vac./(vacc_coverage+0.1), vac_distributed_ts];
    ...
    vac_effect = cumsum([diff(u_vacc_fd./vacc_coverage,1 , 2), vac_distributed_ts(:, 1:end)], 2)*vac_effi;
    vac_effect = vac_effect(:, (size(data_4, 2)-size(vacc_lag_rate, 2)+1):(size(data_4, 2)-size(vacc_lag_rate, 2)+horizon));
    vacc_pre_immunity = [vacc_lag_rate*0 equiv_vacc_effi*((1 - un.*data_4./popu).*u_vacc_fd)];
