...
popu_12 = sum(popu_by_age(:, 3:end), 2) + popu_by_age(:, 2)*0.3;
popu_16 = sum(popu_by_age(:, 4:end), 2) + popu_by_age(:, 3)*0.9;
...
variants_data;
variant_rt_change_list = ones(3, length(popu), size(data_4, 2));

for jj=1:length(sel_idx)
    cid = sel_idx(jj);
    %target_days = [thisday:(thisday+horizon-1)];
    target_days = (1:thisday)';
...
variant_rt_change_list(1, ~sel_idx_bool, :) = repmat(nanmin(variant_rt_change_list(1, sel_idx, :), [], 2), [sum(~sel_idx_bool), 1]);
variant_rt_change_list(3, ~sel_idx_bool, :) = repmat(nanmax(variant_rt_change_list(3, sel_idx, :), [], 2), [sum(~sel_idx_bool), 1]);
variant_rt_change_list(2, ~sel_idx_bool, :) = repmat(nanmedian(variant_rt_change_list(2, sel_idx, :), 2), [sum(~sel_idx_bool), 1]);
...
% Rt_max = max(Rt_mat, [], 2);
Target_Rt = ones(length(popu), 1) * [2.5 2.75 3]*1.5; Rt_final_scens = [1 2 3];
Feb_Rt = ones(length(popu), 1); Rt_init_scens = [1 2];
scenario_forecasts = 1;


variant_fact = squeeze(variant_rt_change_list(2, :, :));
...
vacc_monthly = 50000000;
equiv_vacc_effi = (0.75+0.95)*0.5;
jj_eff = 0.7;
vacc_future = (popu./sum(popu))*ones(1, 200)*vacc_monthly/30.5;
ap_day = days(datetime(2021, 4, 1) - now_date);
vacc_future(:, ap_day:ap_day+29) = vacc_future(:, ap_day:ap_day+29)+ (jj_eff./equiv_vacc_effi)*(popu./sum(popu))*10000000/30;
vacc_future(:, ap_day+30:ap_day+60) = vacc_future(:, ap_day+30:ap_day+60)+ (jj_eff./equiv_vacc_effi)*(popu./sum(popu))*15000000/30;
vacc_future(:, ap_day+61:end) = vacc_future(:, ap_day+61:end) + (jj_eff./equiv_vacc_effi)*(popu./sum(popu))*20000000/30;

vacc_coverage_list = [0.8 0.7];
...
vacc_monthly = 45000000;
equiv_vacc_effi = (0.50+0.85)*0.5;
jj_eff = 0.6;
vacc_future = (popu./sum(popu))*ones(1, 200)*vacc_monthly/30.5;
ap_day = days(datetime(2021, 4, 1) - now_date);
vacc_future = vacc_future + (jj_eff./equiv_vacc_effi)*(popu./sum(popu))*ones(1, 200)*5000000/30;

vacc_coverage_list = [0.55 0.65];
