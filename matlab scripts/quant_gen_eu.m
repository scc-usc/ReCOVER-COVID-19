...
eu_names = cell(length(placenames), 1);
eu_present = zeros(length(placenames), 1);
for jj = 1:length(eu_names_table.location)
    if strcmpi(eu_names_table.location_name(jj), 'Czechia')==1
        idx = find(strcmpi(placenames, 'Czech Republic'));
    else
        idx = find(strcmpi(placenames, eu_names_table.location_name(jj)));
    end
    if ~isempty(idx)
        eu_present(idx(1)) = 1;
...
T1.value = compose('%g', round(thesevals, 1));
...
T2.value = compose('%g', round(thesevals, 1));
...
T3.value = compose('%g', round(thesevals, 1));
