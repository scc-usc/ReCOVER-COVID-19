function [infec] = var_simulate_pred_vac(data_4, passengerFlowDarpa, beta_all_cell, popu, k_l, horizon, jp_l, un_fact, base_infec, vac, rate_change)
...
if length(F) <= 1    % Optimized code when mobility not considered
    for j=1:length(popu)
        ...
        for t=1:horizon
            ...
            Xt = [S*Ikt; 0];
            yt = rate_change(j, t).*sum(beta_all_cell{j}'*Xt);
            yt(yt < 0) = 0;
    ...
    for t=1:horizon
        true_infec = un_fact.*lastinfec;
        S = 1- true_infec./popu - ((1-true_infec./popu).*vac(:, t))./popu;
        ...
        for j=1:length(popu)
            ...
            yt(j) = rate_change(j, t)*sum(beta_all_cell{j}'.*Xt, 2);
        end
        yt(yt<0) = 0;
        lastinfec = lastinfec + yt;
        infec(:, t) = lastinfec;
