function [beta_cell, un_prob, E, initdat, ci, del, fittedC] = learn_un_fix_beta_new(data_4, popu, k_l, jp_l, alpha_l, a_T, min_pts, mode, ret_conf)
...
for j=1:length(popu)
    ...
    if length(thisdata) < jk+3+3    % At least 3 points to learn beta and 3 to learn gamma
        continue;
    end
...
if Ef < 0 || Ef >2 || any(abs(y-fvals)./y) >= 1 || eps >= 1
    del(j) = -1;
end
%%%%%%% Train for gamma
    ...
    elseif mode == 'f'
        ...
        if ret_conf==0
            ...
            mdl = fitnlm(X1, y1, @(w, X)(X*(1+ exp(-w))), zeros(k, 1));
            un_fact = 1 + exp(-mdl.Coefficients.Estimate);
            ci{j} = 1./(1+ exp(-mdl.coefCI(0.01)));
            un_prob(j) = 1./un_fact;
        end
...
%     Et = sum(1-(y-fvals)./y)./(length(y)*(1-eps));
    ...
    if Et < 0 || Et >2 || any(abs(y-fvals)./y) >= 1 || eps >= 1
        del(j) = -1;
    end
...
%%%%%%%%% Identify delta 
    E = Et/Ef;
    g = un_prob(j); 
    %delvec = (a_T*g.*(1-a_f) - a_f.*(g-a_T))./((1-a_f).*E.*g.^2 - g.*(g-a_T));
    temp = E*g*(1-a_f)/(g-a_T);
    delvec = 1 - (a_f - temp*a_T)./(g - temp*g);
    del(j) = delvec;
    if g < a_f || g < a_T
        del(j) = -1;
    end
