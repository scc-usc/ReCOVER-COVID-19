...
for j=1:length(popu)
    ...
    if ret_conf == 1
        beta_diff = abs(beta_CI(2) - beta_CI(1));
        if beta_diff >1
            beta_diff = 1;
        end
    else
    ...
    % Solve quadratic equation to get del
    beta_diff = 0; g_bar = ci{j}(1, 1);
    a = a_T*(1-a_f);
    b = g_bar*(a_f - beta_diff*(1-a_f)) - a_T*(1-a_f);
    c = g_bar*beta_diff*(1-a_f) + a_f*(1-g_bar);
    this_del1 = (-b + sqrt(b^2 - 4*a*c))./(2*a);
    this_del2 = (-b + sqrt(b^2 - 4*a*c))./(2*a);
    %this_del = ((1-a_f - un_prob(j))- sqrt((1-a_f-un_prob(j))^2 - 4*(1-a_f)*(a_f/a_T(j) - un_prob(j))))/(2*(1-a_f));
    if isreal(this_del1)
        if  (this_del2 > 0) && (this_del2 < 1)
            del(j) = this_del2;
        elseif (this_del1 > 0) && (this_del1 < 1)
            del(j) = this_del1;
        else
            del(j) = -1;
        end
