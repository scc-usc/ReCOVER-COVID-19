function [beta_all_cell, un_prob, initdat, fittedC, ci, err] = learn_nonlin(data_4, popu, k_l, jp_l, alpha_l, beta_init, mode, prob_range)
    ...
    func = @(w, x)((1-x(:, 1)/(w(1, 1) - x(:, 2)/(w(2, 1)))).*( x(:, 2:end)*(w(3:end, 1))));
    ...
    for j=1:length(popu)
        ...
        for t = jk+1:maxt-1
            ...
            X(t-jk, :) = [a0, (thisdata(t)./popu(j)-a0), Ikt];
            y(t-jk) = thisinc(t);
        end
        ...
        func_nlm = @(w, x)((1-x(:, 1)/(lb + (ub-lb)*1./(1+exp(-w(1)))) -x(:, 2)/(0.01 + (1-0.01)/(1+exp(-w(2))))).*( x(:, 3:end).*(1./(1+exp(-w(3:end))))));
        %%%%%%%%%%%%%%%%%%
        initdat(j, end-jk:end) = data_4(j, 1:jk+1);
        opts1=  optimset('display','off');
        if mode == 'i'
            mdl = fitnlm(X, y, func_nlm, zeros(k+2, 1));
            unb = mdl.Coefficients.Estimate;
            unb_ci = mdl.coefCI(0.01);
            ww = [lb + (ub-lb)*1./(1+exp(-unb(1))), 0.01+(1-0.01)./(1+exp(-unb(2))), 1./(1+exp(-unb(3:end)))];
            ci{j} = [lb + (ub-lb)*1./(1+exp(-unb_ci(1, :))); 0.01+(1-0.01)./(1+exp(-unb_ci(2:end, :)))];
