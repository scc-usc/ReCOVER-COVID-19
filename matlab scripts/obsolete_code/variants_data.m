...
% func1 = @(st, rr, ii, tvals)(sum((log(ii./(1+exp(-st(1:end-3))) + st(1:end-3).*st(end-2) - tvals.*st(end-1) + st(end))).^2) + sum((st(1:end-3) - log(rr)).^2));
% func2 = @(st, rr)(sum((st(1:end-3) - log(rr)).^2));
% func3 = @(st, rr, ss)(sum((ss.*(1-rr)).*log(1+exp(st(1:end-3))) + (ss.*(rr)).*log(1+exp(-st(1:end-3)))));
% options = optimoptions(@fmincon,'Algorithm', 'sqp', 'MaxFunctionEvaluation', 10000, 'Display','none');
% for jj=1:length(sel_idx)
%     cid = sel_idx(jj); 
%     yy = rat_data(cid, :);
%     nnanidx = intersect(find(~isnan(yy)), find(yy>0.00)); nnanidx(nnanidx<385) = [];
%     rr = yy(nnanidx);
%     ii = diff(data_4_s(cid, :)); ii = ii(nnanidx);
%     ss = u_tot_o(cid, nnanidx);
%     tvals = nnanidx;
%     nn = length(rr);
%     lb = [-Inf(1, nn) -Inf -Inf -Inf];
%     ub = [Inf(1, nn) Inf Inf Inf];
%     
%     A = zeros(2, nn+3); b = zeros(2, 1);
% 
%     try
%         st_out{cid} = fmincon(@(x)(func3(x, rr./(1+rr), ss)), [rr -0.5 0 0], [], [], [], [], lb, ub, @(x)(variant_t_cons(x, rr, ii, tvals)), options);
%         fC_out{cid} = [nnanidx' (rr') exp(st_out{cid}(1:end-3)')];
%         fprintf('.');
%       catch
%          continue;
%      end
