...
beta_notravel_f = var_ind_beta_un(data_4_s(:, 1:T_tr), passengerFlow*0, alpha_l, k_l, 1, popu, jp_l);
...
beta_after_f = var_ind_beta_un(data_4_s(:, 1:T_full), passengerFlow*0, alpha_l, k_l, 1, popu, jp_l);
...
infec = var_simulate_pred_un(data_4_s(:, 1:T_full), passengerFlow*0, beta_after, popu, best_param_list(:, 1), horizon, best_param_list(:, 2), 1);
infec_released = var_simulate_pred_un(data_4_s(:, 1:T_full), passengerFlow*0, beta_notravel, popu, best_param_list_no(:, 1), horizon, best_param_list_no(:, 2), 1);

k_l = MAPEtable_s(1, 1)*ones(length(popu), 1);
jp_l = MAPEtable_s(1, 2)*ones(length(popu), 1);
infec_f = var_simulate_pred_un(data_4(:, 1:T_full), passengerFlow*0, beta_after_f, popu, k_l, horizon, jp_l, 1);

k_l = MAPEtable_notravel_fixed_s(1, 1)*ones(length(popu), 1);
jp_l = MAPEtable_notravel_fixed_s(1, 2)*ones(length(popu), 1);
infec_released_f = var_simulate_pred_un(data_4_s(:, 1:T_full), passengerFlow*0, beta_notravel_f, popu, k_l, horizon, jp_l, 1);
